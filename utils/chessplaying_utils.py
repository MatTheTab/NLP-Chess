...
def generate_next_moves(moves: str,
    ...
    """
    query = "Provide the next move in the chess game." \
        f"Only provide the move, no move numbers. {moves}"
    ...
    for num_moves in tests_num_moves:
        ...
        if plot_hist:
            plot_results_hist(
                ...
                min(avg_bot),
                max(avg_bot),
                f"GPT {num_moves} moves average score difference")
...
def run_legal_forced_test(transcripts: List[str],
                          num_moves: List[int],
                          engine: chess.engine.SimpleEngine,
                          generator: TextGenerationPipeline,
                          forced_generator: TextGenerationPipeline,
                          verbose: bool = False) -> Dict[int,
                                                         Dict[str,
                                                              Dict[str,
                                                                   Any]]]:
    """
    Runs tests for legal forced alternatives using GPT-2
    models and random player.
    ...
    - forced_generator (TextGenerationPipeline): GPT-2 text generation
    pipeline for forced alternatives.
    ...
    player_dict: Dict[str, Any] = {
        "gpt": None, "gpt_forced": None, "random": None, "player": None}
    summaries: Dict[int, Dict[str, Dict[str, Any]]] = dict(
        (x, deepcopy(player_dict)) for x in num_moves)
    ...
    for n_moves in num_moves:
        ...
        scores_random = []
        scores_player = []
        for i, transcript in tqdm(
                enumerate(transcripts), total=len(transcripts)):
            if verbose:
                print("------------------------------------------------------")
                print("Game", i)
            ...
            try:
                answer_forced = generate_next_moves(
                    moves,
                    forced_generator,
                    forced_generator.tokenizer,
                    10,
                    alternatives=legal_moves)[0]
            except ValueError:  # Skip game on issue with `nested_token_ids`
                continue

            try:
                split_answer = answer_forced.split(" ")
                next_moves_forced = [
                    move for move in split_answer if move[0].isalpha()]
            except IndexError:
                # Assume fail if this fails - no clear move in answer
                next_moves_forced = []
            ...
            if len(next_moves_forced) != 0:
                ...
                try:
                    ...
                    evaluation_score = result["score"].pov(
                        color=player).score(mate_score=900)
                    evaluation_diff = evaluation_score - \
                        move_stats['prev_score']
                    ...
                    if verbose:
                        print(
                            f" - Score before move: "
                            f"{move_stats['prev_score']}\n"
                            f" - Score after move: "
                            f"{evaluation_score}\n"
                            f" - Is best move: "
                            f"{next_move_forced==move_stats['best_move']}\n"
                            f" - Is worst move: "
                            f"{next_move_forced==move_stats['worst_move']}\n"
                            f" - Is above average move: "
                            f"{evaluation_diff>move_stats['mean_score']}\n"
                        )
            ...
            answer_normal = generate_next_moves(
                moves, generator, generator.tokenizer, 10)[0]

            try:
                split_answer = answer_normal.split(" ")
                next_moves_normal = [
                    move for move in split_answer if move[0].isalpha()]
            except IndexError:
                # Assume fail if this fails - no clear move in answer
                ...
                try:
                    ...
                    if verbose:
                        print(
                            ...
                            f"{evaluation_score}\n"
                            f" - Is best move: "
                            f"{next_move_normal==move_stats['best_move']}\n"
                            f" - Is worst move: "
                            f"{next_move_normal==move_stats['worst_move']}\n"
                            f" - Is above average move: "
                            f"{evaluation_diff>move_stats['mean_score']}\n"
                        )
            ...
            if forced_move_performed:
                ...
                if verbose:
                    print(
                        ...
                        f"{evaluation_score}\n"
                        f" - Is best move: "
                        f"{next_move_random==move_stats['best_move']}\n"
                        f" - Is worst move: "
                        f"{next_move_random==move_stats['worst_move']}\n"
                        f" - Is above average move: "
                        f"{evaluation_diff>move_stats['mean_score']}\n"
                    )
                ...
                if player_move is not None:
                    ...
                    if verbose:
                        print("PLAYER")
                        print(
                            f"Player move score difference: {evaluation_diff}")
...
def show_results_forced_test(summaries: Dict[int,
                                             Dict[str,
                                                  Dict[str,
                                                       Any]]],
                             tests_num_moves: List[int],
    ...
    Parameters:
    - summaries (Dict[int, Dict[str, Dict[str, Dict[str, Any]]]):
    ...
    for num_moves in tests_num_moves:
        ...
        if len(scores_bot_forced) > 0:
            ...
            print(
                f' - Average score difference after move: '
                f'{avg_score_bot_forced}'
            )

        total_moves = summaries[num_moves]["gpt_forced"]["legal_moves"] \
            + summaries[num_moves]["gpt_forced"]["illegal_moves"]
        print(
            f' - Total number of legal moves: '
            f'{summaries[num_moves]["gpt_forced"]["legal_moves"]} / '
            f'{total_moves}')
        print(
            f' - Total number of best moves: '
            f'{summaries[num_moves]["gpt_forced"]["best_moves"]} / '
            f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
        print(
            f' - Total number of worst moves: '
            f'{summaries[num_moves]["gpt_forced"]["worst_moves"]} / '
            f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
        print(
            f' - Total number of above average moves: '
            f'{summaries[num_moves]["gpt_forced"]["above_average_moves"]} / '
            f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')

        if plot_hist and len(scores_bot_forced) > 1:
            plot_results_hist(
                scores_bot_forced,
                min(scores_bot_forced),
                max(scores_bot_forced),
                f"GPT FORCED ALT {num_moves} moves average score difference")
        ...
        total_moves = summaries[num_moves]["gpt"]["legal_moves"] \
            + summaries[num_moves]["gpt"]["illegal_moves"]
        print(
            f' - Total number of legal moves: '
            f'{summaries[num_moves]["gpt"]["legal_moves"]} / '
            f'{total_moves}')
        print(
            f' - Total number of best moves: '
            f'{summaries[num_moves]["gpt"]["best_moves"]} / '
            f'{summaries[num_moves]["gpt"]["legal_moves"]}')
        print(
            f' - Total number of worst moves: '
            f'{summaries[num_moves]["gpt"]["worst_moves"]} / '
            f'{summaries[num_moves]["gpt"]["legal_moves"]}')
        print(
            f' - Total number of above average moves: '
            f'{summaries[num_moves]["gpt"]["above_average_moves"]} / '
            f'{summaries[num_moves]["gpt"]["legal_moves"]}')
        ...
        if plot_hist and len(scores_bot) > 1:
            plot_results_hist(
                ...
                min(scores_bot),
                max(scores_bot),
                f"GPT NORM {num_moves} moves average score difference")
        ...
        if len(scores_bot_forced) > 0:
            ...
            print(
                f' - Total number of best moves: '
                f'{summaries[num_moves]["random"]["best_moves"]} / '
                f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
            print(
                f' - Total number of worst moves: '
                f'{summaries[num_moves]["random"]["worst_moves"]} / '
                f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
            print(
                f' - Total number of above average moves: '
                f'{summaries[num_moves]["random"]["above_average_moves"]} / '
                f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
        ...
        if len(scores_bot_forced) > 0:
            ...
            print(
                f' - Total number of best moves: '
                f'{summaries[num_moves]["player"]["best_moves"]} / '
                f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
            print(
                f' - Total number of worst moves: '
                f'{summaries[num_moves]["player"]["worst_moves"]} / '
                f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
            print(
                f' - Total number of above average moves: '
                f'{summaries[num_moves]["player"]["above_average_moves"]} / '
                f'{summaries[num_moves]["gpt_forced"]["legal_moves"]}')
            ...
            if plot_hist and len(scores_bot_forced) > 1:
                plot_results_hist(
                    ...
                    min(avg_player),
                    max(avg_player),
                    f"Player {num_moves} moves average score difference")
        else:
            print("No correct forced model moves, no player comparison")

        aggregate_summary_bot_forced["agg_legal"] \
            += summaries[num_moves]["gpt_forced"]["legal_moves"]
        aggregate_summary_bot_forced["agg_illegal"] \
            += summaries[num_moves]["gpt_forced"]["illegal_moves"]
        aggregate_summary_bot_forced["agg_best"] \
            += summaries[num_moves]["gpt_forced"]["best_moves"]
        aggregate_summary_bot_forced["agg_worst"] \
            += summaries[num_moves]["gpt_forced"]["worst_moves"]
        aggregate_summary_bot_forced["agg_above_average"] \
            += summaries[num_moves]["gpt_forced"]["above_average_moves"]

        aggregate_summary_bot["agg_legal"] \
            += summaries[num_moves]["gpt"]["legal_moves"]
        aggregate_summary_bot["agg_illegal"] \
            += summaries[num_moves]["gpt"]["illegal_moves"]
        aggregate_summary_bot["agg_best"] \
            += summaries[num_moves]["gpt"]["best_moves"]
        aggregate_summary_bot["agg_worst"] \
            += summaries[num_moves]["gpt"]["worst_moves"]
        aggregate_summary_bot["agg_above_average"] \
            += summaries[num_moves]["gpt"]["above_average_moves"]

        aggregate_summary_random["agg_best"] \
            += summaries[num_moves]["random"]["best_moves"]
        aggregate_summary_random["agg_worst"] \
            += summaries[num_moves]["random"]["worst_moves"]
        aggregate_summary_random["agg_above_average"] \
            += summaries[num_moves]["random"]["above_average_moves"]

        aggregate_summary_player["agg_best"] \
            += summaries[num_moves]["player"]["best_moves"]
        aggregate_summary_player["agg_worst"] \
            += summaries[num_moves]["player"]["worst_moves"]
        aggregate_summary_player["agg_above_average"] \
            += summaries[num_moves]["player"]["above_average_moves"]

    total_agg_moves_gpt2_f = aggregate_summary_bot_forced["agg_illegal"] \
        + aggregate_summary_bot_forced["agg_legal"]
    print("===============================================================")
    print("Aggregate results GPT-2 FORCED ALTERNATIVES")
    print(
        f' - Average score difference after move: '
        f'{round(np.mean(avg_scores_bot_forced), 3)}')
    print(
        f' - Total number of legal moves: '
        f'{aggregate_summary_bot_forced["agg_legal"]} / '
        f'{total_agg_moves_gpt2_f}')
    print(
        f' - Total number of best moves: '
        f'{aggregate_summary_bot_forced["agg_best"]} / '
        f'{aggregate_summary_bot_forced["agg_legal"]}')
    print(
        f' - Total number of worst moves: '
        f'{aggregate_summary_bot_forced["agg_worst"]} / '
        f'{aggregate_summary_bot_forced["agg_legal"]}')
    print(
        f' - Total number of above average moves: '
        f'{aggregate_summary_bot_forced["agg_above_average"]} / '
        f'{aggregate_summary_bot_forced["agg_legal"]}')

    total_agg_moves_gpt2 = aggregate_summary_bot["agg_illegal"] \
        + aggregate_summary_bot_forced["agg_legal"]
    print(
        "----------------------------------------------------------------")
    ...
    print(
        f' - Average score difference after move: '
        f'{round(np.mean(avg_scores_bot), 3)}')
    print(
        f' - Total number of legal moves: '
        f'{aggregate_summary_bot["agg_legal"]} / '
        f'{total_agg_moves_gpt2}')
    print(
        f' - Total number of best moves: '
        f'{aggregate_summary_bot["agg_best"]} / '
        f'{aggregate_summary_bot["agg_legal"]}')
    ...
    print(
        f' - Average score difference after move: '
        f'{round(np.mean(avg_scores_random), 3)}')
    print(
        f' - Total number of best moves: '
        f'{aggregate_summary_random["agg_best"]} / '
        f'{aggregate_summary_bot_forced["agg_legal"]}')
    ...
    print(
        f' - Average score difference after move: '
        f'{round(np.mean(avg_scores_player), 3)}')
    print(
        f' - Total number of best moves: '
        f'{aggregate_summary_player["agg_best"]} / '
        f'{aggregate_summary_bot_forced["agg_legal"]}')
